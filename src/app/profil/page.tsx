            ...
            <div className="relative z-10 max-w-6xl mx-auto space-y-8">
                <div className="flex flex-col md:flex-row items-center gap-8 glass p-8 rounded-3xl border border-white/5">
                    <div className="relative">
                        <div className="w-32 h-32 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 p-1">
                            <div className="w-full h-full rounded-full bg-slate-900 flex items-center justify-center text-5xl object-cover">👤</div>
                        </div>
                    ...
                    <div className="text-center md:text-left flex-1">
                        ...
                        <p className="text-xs text-slate-500 mt-2 text-right max-w-md">Sonraki seviye: %{Math.round(100 - levelInfo.nextLevelProgress)} kaldı</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1 space-y-8">
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="glass p-6 rounded-3xl border border-white/5">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-bold text-white">Aile Bağlantısı</h3>
                                <span className="px-2 py-1 bg-purple-500/20 text-purple-400 text-xs font-bold rounded-lg border border-purple-500/20">AİLE</span>
                            </div>
                            <div className="space-y-4">
                                <div className="bg-slate-800/50 p-4 rounded-xl flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-500 to-purple-500 flex items-center justify-center text-lg">👯</div>
                                    <div>
                                        <div className="text-sm font-bold text-white">{levelInfo.twinName || 'İkizim'}</div>
                                        <div className="text-xs text-slate-400">{levelInfo.twinName ? 'Bağlı' : 'Bağlantı Bekleniyor...'}</div>
                                    </div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass p-6 rounded-3xl border border-white/5">
                            ...
                            <div className="space-y-3">
                                ...
                                {recentActivities.length === 0 && <p className="text-slate-500 text-xs text-center py-4">Henüz aktivite yok.</p>}
                            </div>
                        </motion.div>
                    </div>

                    <div className="lg:col-span-2 space-y-8">
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass p-8 rounded-3xl border border-white/5 flex flex-col items-center">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass p-8 rounded-3xl border border-white/5">
                            <h3 className="text-xl font-bold text-white mb-6">Rozetler</h3>
                            <div className="flex gap-4 flex-wrap">
                                {stats.totalPoints > 100 && <div className="w-16 h-16 rounded-full bg-yellow-500/20 border border-yellow-500 text-2xl flex items-center justify-center tooltip cursor-help" title="Başlangıç">🚀</div>}
                                {stats.gamesPlayed >= 5 && <div className="w-16 h-16 rounded-full bg-purple-500/20 border border-purple-500 text-2xl flex items-center justify-center tooltip cursor-help" title="Oyun Ustası">🎮</div>}
                                {stats.radarData[0] > 70 && <div className="w-16 h-16 rounded-full bg-green-500/20 border border-green-500 text-2xl flex items-center justify-center tooltip cursor-help" title="Özgür Ruh">🌿</div>}
